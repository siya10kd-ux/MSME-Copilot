...
def _identify_delayed_suppliers(scores: list[dict]) -> list[dict]:
    """Return suppliers with a reliability score below the acceptable threshold.

    Fix #8: The original condition also included `or delayed_deliveries > 0`,
    which flagged almost every supplier (even one late delivery in hundreds
    would qualify). Now we use the reliability_score alone as the gate,
    which already accounts for delay frequency and average delay magnitude.
    """
    return [s for s in scores if s["reliability_score"] < 70]


def run_supplier_agent(state: SharedState) -> SharedState:
    """Run supplier delay detection and reliability analysis."""
    # ── Fix #11: dataset loading is outside try/except so IO errors surface ───
