...
def generate_purchase_orders(
    ...
    for rec in reorder_recommendations:
        ...
        if supplier_row.empty:
            logger.warning("No supplier found for product_id=%s — skipping PO.", product_id)
            continue

        # ── Fix #10: guard against missing product_id in products table ───────
        product_cost_rows = products[products["product_id"] == product_id]["unit_cost"]
        if product_cost_rows.empty:
            logger.warning("No unit_cost found for product_id=%s — skipping PO.", product_id)
            continue

        supplier = supplier_row.iloc[0]
        order_qty = rec.get("order_qty", rec.get("reorder_point", 100))
        unit_cost = float(product_cost_rows.iloc[0])
